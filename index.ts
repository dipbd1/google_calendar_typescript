...
const TOKEN_PATH: string = "./secrets/token.json";

fs.readFile(
  "./secrets/client_secret.json",
  (err: NodeJS.ErrnoException, content: Buffer | String): void => {
 ...
 */
function authorize(credentials: Credentials, callback: Function): void {
  const {
    client_secret,
    client_id,
    redirect_uris,
  ...
  fs.readFile(TOKEN_PATH, (err: NodeJS.ErrnoException, token: Buffer): void => {
  ...
  calendar.events.list(
    ...
    (err, res) => {
      ...
      if (events.length) {
        ...
        events.map((event, i) => {
          const start: string | Date = event.start.dateTime || event.start.date;
